...
struct EmojiArtDocumentView: View {
    @ObservedObject var documentVM: EmojiArtDocument
    @Environment(\.undoManager) var undoManager
    @ScaledMetric var defaultEmojiFontSize: CGFloat = 42
    ...
    var documentBody: some View {
        GeometryReader { geometry in
            ZStack {
                Color.white
                    OptionalImage(uiImage: documentVM.backgroundImage)
                        .scaleEffect(zoomScale)
                        .position(convertFromEmojiCoordinates((0,0), in: geometry))
                    .gesture(doubleTapToZoom(in: geometry.size))
                if documentVM.backgroundImageFetchStatus == .fetching{
                    ProgressView().scaleEffect(2)
                } else {
                    ForEach(documentVM.emojis) {
            ...
            .onChange(of: documentVM.backgroundImageFetchStatus){
                status in
                switch status {
                    case .failed(let url):
            ...
            .onReceive(documentVM.$backgroundImage){
                image in
                if autoZoom {
            ...
            .compactableToolBar {
                ...
                if PhotoLibrary.isAvailable {
                    AnimatedActionButton(title: "Pick photo", systemImage: "photo"){
    ...
    private func handlePickedBackgroundImage(_ image: UIImage?){
        ...
        if let imageData = image?.jpegData(compressionQuality: 1.0) {
            documentVM.setBackground(.imageData(imageData), undoManager: undoManager)
        }
    ...
    private func pasteBackground() {
        ...
        if let imageData = UIPasteboard.general.image?.jpegData(compressionQuality: 1.0) {
            ...
            documentVM.setBackground(.url(url), undoManager: undoManager)
        } else {
...
//    MARK: - Drag and Drop
    ...
    private func drop(providers: [NSItemProvider], at location: CGPoint, in geometry: GeometryProxy) -> Bool {
        var found = providers.loadObjects(ofType: URL.self) {
            url in
            autoZoom = true
            documentVM.setBackground(EmojiArtModel.Background.url(url.imageURL), undoManager: undoManager)
        }
        ...
        if !found {
            found = providers.loadObjects(ofType: UIImage.self){
                ...
                if let data = image.jpegData(compressionQuality: 1.0){
                    autoZoom = true
                    documentVM.setBackground(.imageData(data), undoManager: undoManager)
                }
            }
        }
        ...
        if !found {
            found = providers.loadObjects(ofType: String.self){
                ...
                if let emoji = string.first, emoji.isEmoji {
                    documentVM.addEmoji(String(emoji),
...
//    MARK: - Zooming
    ...
    private func doubleTapToZoom(in size: CGSize) -> some Gesture {
        TapGesture(count: 2)
            .onEnded{
                withAnimation{
                    zoomToFit(documentVM.backgroundImage, in: size)
                }
            }
    }
...
struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        EmojiArtDocumentView(documentVM: EmojiArtDocument())
    }
}
